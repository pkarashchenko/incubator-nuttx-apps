/****************************************************************************
 * apps/examples/blynk/blynk_mqttc.c
 *
...
#endif
  uint8_t sendbuf[CONFIG_EXAMPLES_BLYNK_MQTTC_TXSIZE];
  uint8_t recvbuf[CONFIG_EXAMPLES_BLYNK_MQTTC_RXSIZE];
  uint32_t tmo;
  uint8_t flags;
  uint8_t qos;
};
                     ...
                     FAR struct mqtt_conn_context_s *conn)
{
  ...
  if (cfg->ca_file)
    ...
    {
      ret = mbedtls_x509_crt_parse(ca_crt, TEST_CA_CRT_RSA_SHA256_PEM,
...
{
  struct mqtt_conn_context_s conn;
  int timeout = 100;
  enum MQTTErrors mqtterr;
  pthread_attr_t attr;
  pthread_t thrdid;
  int n = 5;
  static struct mqttc_cfg_s mqtt_cfg =
    {
      .host = "fra1.blynk-qa.com",
#ifndef MQTT_USE_MBEDTLS
      .port = "1883",
#else
      .port = "8883",
      ...
      .id = "",
      .user = "device",
      .pass = CONFIG_EXAMPLES_BLYNK_AUTH_TOKEN,
      .qos = MQTT_PUBLISH_QOS_0
    };

  parsearg(argc, argv, &mqtt_cfg, &n);

  if (init_conn(&mqtt_cfg, &conn) < 0)
    {
      return -1;
    }

  mqtterr = mqtt_init(&mqtt_cfg.client, conn.handle,
  ...
  if (pthread_create(&thrdid, &attr, client_refresher, &mqtt_cfg.client))
    {
